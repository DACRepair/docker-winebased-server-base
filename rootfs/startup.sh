...
if [ ! -x "$HOME/.config/pcmanfm/LXDE/" ]; then
    ...
    chown -R $USER:$USER $HOME
    su - $USER -c "xdg-user-dirs-update --force"
fi
...
# run xdg to create default user directory's before we call customs scripts so wine symlinks work.

su - $USER -c "/usr/bin/xdg-user-dirs-update --force"


# run scripts for non base-images

if [ -d "/scripts" ] 
then
    bash /scripts/*.sh 
else
    echo "Skipping: Directory does not exists..."
fi
